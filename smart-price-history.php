...
// load translations
add_action('init', function() {	load_plugin_textdomain(		'smart-price-history'
		, false
...
register_deactivation_hook(__FILE__, function() {	Plugin::deactivate();
...
function sph_post_data() {	Plugin::savePostData();
	
} 
/**
 * Add custom menu option
 */
add_action('admin_menu', function() {	Plugin::addMenu();

});

// custom CRON schedules
add_filter('cron_schedules', function($schedules) { 
	$schedules['smart_price_history_export_interval'] = array(		'interval'  => 24 * 60 * 60,     // 24h
		'display'   => 'Smart price history - export',
	);

	$schedules['smart_price_history_check_interval'] = array(		'interval'  => 1 * 60,     // 1m
...
// export action hook
add_action('smart_price_history_export', function() {	Plugin::export();

});

// CRON smart price history export
if (!wp_next_scheduled( 'smart_price_history_export' )) {	$schedule_time = (new DateTime('now'))->setTime(23, 59, 59);

	if ($schedule_time->getTimestamp() < time()) {		$schedule_time->setDate(			(int) getdate()['year'],
			(int) getdate()['mon'],
			(int) getdate()['mday'] + 1
		);
	} 
...
// check action hook
add_action('smart_price_history_check', function() {	Plugin::checkExport();

});

// CRON smart price history check
if (!wp_next_scheduled( 'smart_price_history_check' )) {	$schedule_time = (new DateTime('now'));
	$schedule_time->setTime(		(int) getdate($schedule_time->getTimestamp())['hours'], 
...
add_action('woocommerce_single_product_summary', function() {	Plugin::renderLowestPrice();

}, 11, 0);

if (!function_exists('sph_get_lowest_price')) {	function sph_get_lowest_price() {		global $wpdb, $product;

		$sql = "SELECT MIN(price)			FROM `smart_price_history`
